 ...
 * This class is modular, SOLID-compliant, and does not use tags/genres.
 */
class ThesisModel
{
    private PDO $pdo;
    ...
    private function normalizeTitle(string $title): string
    {
        ...
        return $this->smartTitleCase($title);
    }
    ...
    public function getById(int $thesisId): ?array
    {
        $stmt = $this->pdo->prepare("
            SELECT
                ...
                t.created_at,
                t.updated_at,
                GROUP_CONCAT(DISTINCT CONCAT(a.first_name, ' ', a.last_name) ORDER BY ta.author_order SEPARATOR ', ') AS author
            FROM tbl_theses t
            LEFT JOIN tbl_thesis_authors ta ON ta.thesis_id = t.thesis_id
            LEFT JOIN tbl_authors a ON a.author_id = ta.author_id
            WHERE t.thesis_id = ?
            GROUP BY t.thesis_id, t.accession_no, t.title, t.year, t.created_at, t.updated_at
        ");
    ...
    public function listTheses(int $limit, int $offset, ?string $titleSearch = null, ?int $year = null): array
    {
        ...
        $sql = "
            SELECT
                ...
                GROUP_CONCAT(DISTINCT CONCAT(a.first_name, ' ', a.last_name) ORDER BY ta.author_order SEPARATOR ', ') AS author,
                COALESCE(CONCAT_WS(' ',
                    CASE WHEN cn.shelf_number = 'THESIS' THEN 'TH'
                         WHEN cn.shelf_number = 'TH' THEN 'TH'
                         ELSE COALESCE(NULLIF(cn.shelf_number, ''), NULL)
                    END,
                    CASE WHEN cn.classification_code = 'TH' THEN NULL
            ...
            LEFT JOIN tbl_authors a ON a.author_id = ta.author_id
            $whereSql
            GROUP BY t.thesis_id, t.accession_no, t.title, t.year,
                     cn.shelf_number, cn.classification_code, cn.classification_number, cn.cutter, cn.year
    ...
    {
        $sql = "SELECT COUNT(DISTINCT thesis_id) as total FROM tbl_theses";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
    ...
    public function getCoverPathForThesis(int $thesisId): ?string
    {
        ...
        return "/assets/theses_covers/{$callNumber}.webp";
    }
}
